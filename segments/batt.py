...
def battery(pl, format='{ac_state} {capacity:3.0%}', steps=5, gamify=False,
        full_heart='O', empty_heart='O', online='C', offline='N'):
  ...
  if gamify:
    ...
    hightlight_group = 'battery'
    if ac_powered or capacity > 80:
        hightlight_group = 'battery_full'
    elif capacity < 30:
        hightlight_group = 'battery_empty'
    ret.append({
      'contents': format.format(ac_state=(online if ac_powered else offline), capacity=(capacity / 100.0)),
      'highlight_groups': [hightlight_group],
